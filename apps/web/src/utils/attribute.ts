export const initialTags = [
  {
    id: 1,
    name: 'Best Sellers',
    slug: 'best-sellers',
  },
  {
    id: 2,
    name: 'New Arrivals',
    slug: 'new-arrivals',
  },
  {
    id: 3,
    name: 'Flash Sale',
    slug: 'flash-sale',
  },
  {
    id: 4,
    name: 'Eid Collection',
    slug: 'eid-collection',
  },
  {
    id: 5,
    name: 'Ramadan Sale',
    slug: 'ramadan-sale',
  },
];

export const initialBrands = [
  {
    id: 1,
    name: 'Uniqlo',
    slug: 'uniqlo',
    logo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/9/92/UNIQLO_logo.svg/250px-UNIQLO_logo.svg.png',
    bio: 'Uniqlo is a Japanese clothing brand known for its minimalist design, high-quality essentials, and innovative fabric technology. Founded in 1984 in Hiroshima, Japan, it is part of the Fast Retailing group.',
    createdAt: '2026-02-28T06:38:00.758Z',
  },
  {
    id: 2,
    name: 'Suko',
    slug: 'suko',
    createdAt: '2026-02-28T06:38:00.758Z',
  },
  {
    id: 3,
    name: 'Adidas',
    slug: 'adidas',
    createdAt: '2026-02-28T06:38:00.758Z',
  },
  {
    id: 4,
    name: 'Nike',
    slug: 'nike',
    createdAt: '2026-02-28T06:38:00.758Z',
  },
  {
    id: 5,
    name: 'Zara',
    slug: 'zara',
    createdAt: '2026-02-28T06:38:00.758Z',
  },
  {
    id: 6,
    name: 'H&M',
    slug: 'handm',
    createdAt: '2026-02-28T06:38:00.758Z',
  },
  {
    id: 7,
    name: 'Skechers',
    slug: 'skechers',
    createdAt: '2026-02-28T06:38:00.758Z',
  },
  {
    id: 8,
    name: 'Nevada',
    slug: 'nevada',
    createdAt: '2026-02-28T06:38:00.758Z',
  },
  {
    id: 9,
    name: 'Cardinal',
    slug: 'cardinal',
    createdAt: '2026-02-28T06:38:00.758Z',
  },
  {
    id: 10,
    name: 'Puma',
    slug: 'puma',
    createdAt: '2026-02-28T06:38:00.758Z',
  },
  {
    id: 11,
    name: 'Vans',
    slug: 'vans',
    createdAt: '2026-02-28T06:38:00.758Z',
  },
  {
    id: 23,
    name: 'Fila',
    slug: 'fila',
    createdAt: '2026-03-02T14:52:07.828Z',
  },
];

export const initialAttributes = [
  {
    id: 1,
    name: 'Color',
    slug: 'color',
    attributeValues: [
      {
        id: 43,
        value: 'Black',
        slug: 'black',
        hexUrl: '#343434',
      },
      {
        id: 44,
        value: 'White',
        slug: 'white',
        hexUrl: '#FAF9F6',
      },
      {
        id: 45,
        value: 'Navy',
        slug: 'navy',
        hexUrl: '#000080',
      },
      {
        id: 46,
        value: 'Brown',
        slug: 'brown',
        hexUrl: '#6F4E37',
      },
      {
        id: 47,
        value: 'Olive',
        slug: 'olive',
        hexUrl: '#808000',
      },
      {
        id: 48,
        value: 'Grey',
        slug: 'grey',
        hexUrl: '#A9A9A9',
      },
      {
        id: 49,
        value: 'Wine',
        slug: 'wine',
        hexUrl: '#722F37',
      },
      {
        id: 50,
        value: 'Red',
        slug: 'red',
        hexUrl: '#DC143C',
      },
      {
        id: 51,
        value: 'Maroon',
        slug: 'maroon',
        hexUrl: '#800000',
      },
      {
        id: 52,
        value: 'Camel',
        slug: 'camel',
        hexUrl: '#C19A6B',
      },
      {
        id: 53,
        value: 'Green',
        slug: 'green',
        hexUrl: '#008000',
      },
      {
        id: 54,
        value: 'Blue',
        slug: 'blue',
        hexUrl: '#0096FF',
      },
      {
        id: 64,
        value: 'Beige',
        slug: 'beige',
        hexUrl: '#F5F5DC',
      },
      {
        id: 65,
        value: 'Khaki',
        slug: 'khaki',
        hexUrl: '#F0E68C',
      },
      {
        id: 66,
        value: 'Sage',
        slug: 'sage',
        hexUrl: '#8A9A5B',
      },
      {
        id: 67,
        value: 'Orange',
        slug: 'orange',
        hexUrl: '#FF5F1F',
      },
      {
        id: 68,
        value: 'Pink',
        slug: 'pink',
        hexUrl: '#FF69B4',
      },
      {
        id: 69,
        value: 'Salmon',
        slug: 'salmon',
        hexUrl: '#FA8072',
      },
      {
        id: 98,
        value: 'Violet',
        slug: 'violet',
        hexUrl: '#7F00FF',
      },
    ],
  },
  {
    id: 2,
    name: 'Size',
    slug: 'size',
    attributeValues: [
      {
        id: 1,
        value: '2XS',
        slug: '2xs',
      },
      {
        id: 2,
        value: 'XS',
        slug: 'xs',
      },
      {
        id: 3,
        value: 'S',
        slug: 's',
      },
      {
        id: 4,
        value: 'M',
        slug: 'm',
      },
      {
        id: 5,
        value: 'L',
        slug: 'l',
      },
      {
        id: 6,
        value: 'XL',
        slug: 'xl',
      },
      {
        id: 7,
        value: '2XL',
        slug: '2xl',
      },
      {
        id: 8,
        value: '3XL',
        slug: '3xl',
      },
      {
        id: 9,
        value: '4XL',
        slug: '4xl',
      },
      {
        id: 10,
        value: '25',
        slug: '25',
      },
      {
        id: 11,
        value: '26',
        slug: '26',
      },
      {
        id: 12,
        value: '27',
        slug: '27',
      },
      {
        id: 13,
        value: '28',
        slug: '28',
      },
      {
        id: 14,
        value: '29',
        slug: '29',
      },
      {
        id: 15,
        value: '30',
        slug: '30',
      },
      {
        id: 16,
        value: '31',
        slug: '31',
      },
      {
        id: 17,
        value: '32',
        slug: '32',
      },
      {
        id: 18,
        value: '33',
        slug: '33',
      },
      {
        id: 19,
        value: '34',
        slug: '34',
      },
      {
        id: 20,
        value: '35',
        slug: '35',
      },
      {
        id: 21,
        value: '35.5',
        slug: '355',
      },
      {
        id: 22,
        value: '36',
        slug: '36',
      },
      {
        id: 23,
        value: '36.5',
        slug: '365',
      },
      {
        id: 24,
        value: '37',
        slug: '37',
      },
      {
        id: 25,
        value: '37.5',
        slug: '375',
      },
      {
        id: 26,
        value: '38',
        slug: '38',
      },
      {
        id: 27,
        value: '38.5',
        slug: '385',
      },
      {
        id: 28,
        value: '39',
        slug: '39',
      },
      {
        id: 29,
        value: '39.5',
        slug: '395',
      },
      {
        id: 30,
        value: '40',
        slug: '40',
      },
      {
        id: 31,
        value: '40.5',
        slug: '405',
      },
      {
        id: 32,
        value: '41',
        slug: '41',
      },
      {
        id: 33,
        value: '41.5',
        slug: '415',
      },
      {
        id: 34,
        value: '42',
        slug: '42',
      },
      {
        id: 35,
        value: '42.5',
        slug: '425',
      },
      {
        id: 36,
        value: '43',
        slug: '43',
      },
      {
        id: 37,
        value: '43.5',
        slug: '435',
      },
      {
        id: 38,
        value: '44',
        slug: '44',
      },
      {
        id: 39,
        value: '44.5',
        slug: '445',
      },
      {
        id: 40,
        value: '45',
        slug: '45',
      },
      {
        id: 41,
        value: '46.5',
        slug: '465',
      },
      {
        id: 42,
        value: '47',
        slug: '47',
      },
    ],
  },
  {
    id: 3,
    name: 'Material',
    slug: 'material',
    attributeValues: [],
  },
  {
    id: 4,
    name: 'Motif',
    slug: 'motif',
    attributeValues: [],
  },
];
