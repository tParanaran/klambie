  ...
  return (
    <nav
      ...
      className={`fixed left-3 sm:left-5 transition-all duration-200 py-1.5 md:hidden rounded-full overflow-x-scroll scrollbar-hide
        ...
        } ${isScroll ? 'bg-secondary-opacity  backdrop-blur-xl shadow-xs' : ''}`}
    >
      <div className="flex space-x-1">
        {categories.map((cat) => (
          <Link
            key={cat.slug}
            href={`/c/${cat.slug}`}
            className={`px-3 py-2  whitespace-nowrap rounded-lg uppercase text-xs sm:text-sm font-semibold ${pathname.includes(`/c/${cat.slug}`) ? 'bg-dark dark:bg-orange-800 text-light' : 'bg-black/15 dark:bg-white/10 '}`}
          >
            {cat.name}
          </Link>
        ))}
