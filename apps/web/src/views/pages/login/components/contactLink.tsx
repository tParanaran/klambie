...
const span = 'rounded-full text-dark bg-light p-2 mx-1 text-2xl';
const h1 = 'uppercase mr-5 sm:ml-5';

export default function ContactLink() {
  return (
    <div className={div}>
      <Link href={'/register'} className={`bg-light text-dark ${link}`}>
        <span className={span} aria-label="Register">
          <IoArrowDown />
        </span>
      ...
      <Link href={'/contact'} className={`border ${link}`}>
        <span className={span} aria-label="Contact us">
          <IoMailOutline />
        </span>
